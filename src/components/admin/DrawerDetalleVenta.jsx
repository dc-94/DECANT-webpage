          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group flex flex-col md:flex-row gap-4 md:gap-6 items-start md:items-center">
            ...
            <div className="flex-1 md:border-r border-slate-100 md:pr-4 w-full">
              ...
              {pedido.formData?.telefono ? (
                <a 
                  href={`https://wa.me/${formatWhatsApp(pedido.formData.telefono)}`} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 bg-[#25D366] text-white px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest hover:bg-[#1ebe5d] transition-colors"
                >
            ...
            <div className="flex-1 w-full relative">
              ...
              {pedido.formData?.referencia && (
                <p className="text-[9px] text-slate-500 italic mt-1">Ref: {pedido.formData.referencia}</p>
              )}
              ...
              <div className="flex flex-col gap-2">
                ...
                {pedido.estado !== 'Pagado' && (
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-1.5 py-1.5 bg-slate-100 text-slate-500 hover:bg-slate-200 rounded-md text-[8px] font-black uppercase tracking-widest transition-colors outline-none"
                  >
              ...
              {pedido.cart?.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center p-3 bg-white border border-slate-100 rounded-lg shadow-sm">
                  ...
                  <span className="font-black text-xs text-slate-900 shrink-0">
                    ${((item.precioFinal || item.precio) * item.cantidad).toLocaleString()}
                  </span>
                </div>
              ))}
            </div>
