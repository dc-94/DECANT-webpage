...
export default function Suscripciones() {
  ...
  // 2. Buscamos la palabra clave en el NOMBRE (porque no tienen subcategoría)
    ...
    <div className="min-h-screen bg-neutral-white font-poppins text-extra-black">
      ...
      <section className="py-24 bg-[#F8F9FA]">
        <div className="max-w-[70rem] mx-auto px-6">
          {/* Contenedor Grid con items-stretch para forzar misma altura */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-stretch">
            ...
            <div className="bg-white p-8 md:p-12 border border-light-blue/10 shadow-sm flex flex-col h-full">
              ...
              {/* IMAGEN 1:1 (Más chica y cuadrada) */}
                ...
                {planDescorche?.precioFinal ? (
                  <>
                    <span className="font-poppins text-3xl font-black text-brand-orange tracking-tight">
                      ${planDescorche.precioFinal.toLocaleString('es-AR')}
                    </span>
              ...
              <button 
                onClick={() => handleAgregarSuscripcion(planDescorche)}
                disabled={!planDescorche} 
                className="w-full bg-extra-black text-white py-4 text-[10px] font-black uppercase tracking-widest hover:bg-brand-orange transition-colors outline-none disabled:opacity-50 disabled:cursor-not-allowed mt-auto"
              >
            ...
            <div className="bg-extra-black p-8 md:p-12 border border-extra-black shadow-2xl flex flex-col h-full relative">
              ...
              {planTerruno?.imageUrl ? (
                ...
                <div className="w-40 h-40 md:w-48 md:h-48 mx-auto bg-white/5 flex items-center justify-center mb-8 shrink-0 rounded-sm">
                   <span className="text-xs text-light-blue/50 uppercase tracking-widest">Sin Imagen</span>
                </div>
              )}
              ...
              <div className="mb-8 text-center md:text-left">
                {planTerruno?.precioFinal ? (
                  <>
                    <span className="font-poppins text-3xl font-black text-white tracking-tight">
                      ${planTerruno.precioFinal.toLocaleString('es-AR')}
                    </span>
                    <span className="text-xs font-bold text-light-blue ml-2 uppercase tracking-widest">/ mes</span>
                  </>
                ) : (
              ...
              <button 
                onClick={() => handleAgregarSuscripcion(planTerruno)}
                disabled={!planTerruno}
                className="w-full bg-brand-orange text-white py-4 text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-extra-black transition-colors outline-none disabled:opacity-50 disabled:cursor-not-allowed mt-auto"
              >
