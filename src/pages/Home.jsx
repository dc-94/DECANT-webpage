...
export default function Home() {
  ...
  // BENTO BOX DINÁMICO: Tamaños aleatorios y bloques transparentes
  ...
  const deli = storefront?.seccionDeli || {};
  ...
  return (
    <div className="min-h-screen bg-neutral-white overflow-x-hidden font-poppins">
      ...
      {bentoItems.length > 0 && (
        <section className="w-full bg-extra-black p-2 md:p-4">
          <div className="grid grid-cols-2 md:grid-cols-4 auto-rows-[200px] md:auto-rows-[250px] gap-2 md:gap-4 grid-flow-dense">
            {bentoItems.map((item) => {
              if (item.type === 'empty') {
                return <div key={item.id} className={`${item.span} bg-transparent border border-white/5`}></div>;
              }
              return (
      ...
      {/* SECCIÓN DELI CON RECETAS */}
      <section className={`w-full py-24 transition-colors duration-1000 ${deli.tema === 'cafe' ? 'bg-[#FAF7F2]' : 'bg-[#F2F4F0]'}`}>
        <div className="max-w-[90rem] mx-auto px-6 flex flex-col md:flex-row items-center gap-16">
          ...
          <div className="w-full md:w-1/2 relative z-10">
             ...
             <h2 className="text-dark-blue font-playfair font-black uppercase text-3xl md:text-5xl italic mb-8 leading-tight">{deli.tituloReceta}</h2>
             <div className="font-poppins text-sm text-dark-blue/70 leading-relaxed mb-10 whitespace-pre-wrap">{deli.textoReceta}</div>
             
             <div className="flex flex-col sm:flex-row gap-4">
               <Link to={deli.linkProducto || "/shop/deli"} className="bg-extra-black text-brand-white px-10 py-4 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-brand-orange transition-all text-center">
                  Comprar Ingredientes
               </Link>
               {deli.botonSecundarioTexto && (
                 <Link to={deli.botonSecundarioLink || "/shop"} className="border border-extra-black text-extra-black px-10 py-4 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-extra-black hover:text-brand-white transition-all text-center">
                    {deli.botonSecundarioTexto}
                 </Link>
               )}
             </div>
