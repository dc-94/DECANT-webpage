...
export default function ProductDetail() {
  ...
  const productosRelacionados = useMemo(() => {
    ...
    if (relacionados.length < 3) relacionados = [...relacionados, ...catalogoLimpio.filter(p => p.categoria === producto.categoria && !relacionados.includes(p))];
    if (relacionados.length < 3) relacionados = [...relacionados, ...catalogoLimpio.filter(p => !relacionados.includes(p))];
  ...
  const getTruncatedBreadcrumb = () => {
    ...
    const cat = producto.categoria; const sub = producto.subcategoria; const cepa = producto.varietal || producto.cepa; 
    ...
    if (segments.length > 4) {
      items.push(<span key="ellips" className="opacity-60">...</span>); items.push(<span key="sep-ellips" className="mx-1">/</span>);
      const activePath = segments.slice(-2);
      activePath.forEach((seg, index) => { items.push(<Link key={seg.link} to={seg.link} className="hover:text-brand-orange transition-colors">{seg.label}</Link>); if (index === 0) items.push(<span key={`sep-active-${index}`} className="mx-1">/</span>); });
    } else {
      segments.forEach((seg, index) => { items.push(<Link key={seg.link} to={seg.link} className="hover:text-brand-orange transition-colors">{seg.label}</Link>); if (index < segments.length - 1) items.push(<span key={`sep-${index}`} className="mx-1">/</span>); });
    }
    return items;
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-white overflow-x-hidden">
      <style>{`.hide-scrollbar::-webkit-scrollbar { display: none; } .hide-scrollbar { -ms-overflow-style: none; scrollbar-width: none; } @keyframes swipe-bounce { 0%, 100% { transform: translateX(0); } 50% { transform: translateX(25%); } } .animate-swipe-bounce { animation: swipe-bounce 1.5s infinite ease-in-out; }`}</style>
      ...
      <main className="max-w-[95rem] mx-auto grid grid-cols-1 lg:grid-cols-2 items-start pt-6 lg:pt-10 pb-20 lg:pb-0 relative z-10">
        <section className="relative h-[40vh] lg:h-[75vh] flex items-center justify-center overflow-hidden border-b lg:border-b-0 lg:border-r border-dark-blue/5 lg:sticky lg:top-24 lg:pb-8 lg:pl-16 lg:pr-16 z-10">
          <div className="absolute bottom-6 md:bottom-auto md:top-6 left-0 w-full flex justify-center z-10 pointer-events-none">
            {producto.aPedido ? <p className="text-dark-blue text-[10px] md:text-[12px] font-poppins font-black uppercase tracking-[0.2em] drop-shadow-sm bg-white/70 px-2 py-0.5 rounded backdrop-blur-sm">A Pedido</p> : (stock > 0 && stock <= 3) ? <p className="text-brand-orange text-[10px] md:text-[12px] font-poppins font-black uppercase tracking-[0.2em] animate-pulse drop-shadow-sm bg-white/70 px-2 py-0.5 rounded backdrop-blur-sm">{leyendaStock}</p> : null}
          </div>
          <div className="absolute inset-0 flex items-center justify-center opacity-10 z-0 pointer-events-none"><div className="w-[80%] h-[80%] animate-spin-slow"><BlobProducto className="w-full h-full text-light-blue" /></div></div>
          {producto.imageUrl ? <img src={producto.imageUrl} alt={producto.nombre} className={`relative z-10 h-[70%] lg:h-auto max-h-[65%] lg:max-h-[70%] w-auto object-contain drop-shadow-product transition-transform duration-1000 hover:scale-105 ${sinStock ? 'grayscale opacity-40' : ''}`} /> : <span className="font-playfair italic opacity-20 text-3xl z-10 relative">Selección Decant</span>}
        </section>
              ...
              {descuentoVIPAplicado ? (
                ...
                mostrarLabelNormal && (
                  <div className="flex flex-row items-center justify-center bg-brand-orange text-brand-white shadow-md">
                    ...
                    <span className={`text-[9px] md:text-[10px] font-black uppercase px-3 py-1.5 tracking-widest leading-tight ${producto.descuentoPorcentaje > 0 ? 'border-l border-brand-white/20' : ''}`}>{producto.descuentoNombre}</span>
                  </div>
                )
              )}
            </div>
        ...
        <section className="w-full bg-[#F4F7FA] mt-12 py-16 md:py-24 relative z-10 overflow-hidden">
          <div className="max-w-[95rem] mx-auto px-0 md:px-6 lg:px-20">
            ...
            <div className="md:hidden flex items-center justify-end gap-2 text-brand-orange text-[9px] font-black uppercase tracking-widest px-6 mb-4 opacity-80">
              <span>Deslizá para ver</span><svg className="w-4 h-4 animate-swipe-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
            </div>
            <div className="flex overflow-x-auto md:grid md:grid-cols-3 gap-4 md:gap-12 lg:gap-24 px-6 md:px-0 lg:px-12 snap-x snap-mandatory hide-scrollbar pb-8 md:pb-0">
              {productosRelacionados.map(prod => (
                <div key={prod.id} className="flex-none w-[75vw] sm:w-[60vw] md:w-auto md:flex-auto snap-center md:snap-align-none"><ProductCard producto={prod} /></div>
              ))}
            </div>
          </div>
        </section>
